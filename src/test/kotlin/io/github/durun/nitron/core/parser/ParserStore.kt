...
object ParserStore {
	...
	fun getOrThrow(grammarFiles: Collection<Path>, utilJavaFiles: Collection<Path> = emptySet()): CommonParser {
	...
	fun getOrNull(grammarFiles: Collection<Path>, utilJavaFiles: Collection<Path> = emptySet()): CommonParser? {
		val key = grammarFiles.toSet() to utilJavaFiles.toSet()
		return synchronized(parsers) {
			parsers.computeIfAbsent(key) { mapping(key) }
		}.getOrNull()
